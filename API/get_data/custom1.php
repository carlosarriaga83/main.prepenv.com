...
function FUNCION($DATA){
	...
	/// CURRENT DATA 
			...
			$currentQuery = sprintf("SELECT * FROM %s WHERE JSON_EXTRACT(Datos, '$.EDIT_ID') = '%s'", $DATA['TABLA'], $DATA['EDIT_ID']);
			//echo $currentQuery;
	...
	/// UPDATE THE DATABASE  
			...
			$updateQuery = sprintf("UPDATE %s SET Datos = '%s' WHERE JSON_EXTRACT(Datos, '$.EDIT_ID') = '%s' ", $DATA['TABLA'], $updatedJson, $DATA['ID']);
			$R1 = SQL_2_OBJ_V2($updateQuery, $DATA['DB_ID']); // Execute the update
	
	
	/// UPDATED DATA 
	
			$currentQuery = sprintf("SELECT * FROM %s WHERE JSON_EXTRACT(Datos, '$.EDIT_ID') = '%s'", $DATA['TABLA'], $DATA['ID']);
			//echo $currentQuery;
			$currentData = SQL_2_OBJ_V2($currentQuery, $DATA['DB_ID']);
			$currentData = $currentData['PL'][0];

			//$RESP['R1'] = $R1; // Store the result of the update
			$RESP['PL'] = $currentData; // Optionally return the updated data
