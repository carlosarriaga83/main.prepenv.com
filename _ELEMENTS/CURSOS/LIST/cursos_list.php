	...
	/// BARRA DE BUSQUEDA ///
		...
		$htmlOutput .= <<<H
						...
						<div class="card-header border-bottom bg-base py-4 px-24 d-flex align-items-center flex-wrap gap-3 justify-content-between" style="justify-content: flex-end !important;">
							...
							<a href="{$T_PARAMS['TABLE']['ADD_NEW']['href']}" class="btn btn-primary text-sm btn-sm px-12 py-12 radius-8 d-flex align-items-center gap-2 add"  data-bs-toggle="modal" data-bs-target="#Modal_1" >
				...
				if (true ) {
					// Append each value wrapped in <td> tags to the current row
						...
						switch ($col) {
							...
							case "consumo":
								if (isset($row['unidades']) && $row['unidades'] != 0) {
									$consumo = ($row['usadas'] / $row['unidades']) * 100;
								} else {
									$consumo = 0; // Default to 0 if unidades is 0 or not set to avoid division by zero
								}
								$bar_color = 'primary';
								...
								$htmlOutput .= <<<H
												<td class="text-center ">
													<div class="d-flex align-items-center gap-10 justify-content-center">
														<button type="button" class="bg-info-focus bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle" data-bs-toggle="modal" data-bs-target="#Modal_1" onclick="LOAD_ID({$row['EDIT_ID']});">
															<iconify-icon icon="majesticons:eye-line" class="icon text-xl"></iconify-icon>
														</button>

															<button type="button" class="bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle edit admin_edit" EDIT_ID="{$row['EDIT_ID']}" {$T_PARAMS['TABLE']['ADD_NEW']['MISC_PROPS']} onclick="LOAD_ID({$row['EDIT_ID']});">
																<iconify-icon icon="lucide:edit" class="menu-icon"></iconify-icon>
															</button>

														
															<button type="button" class="bg-danger-focus text-danger-600 bg-hover-danger-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle remove-item-btn delete admin_delete" onclick="DELETE_ID('{$T_PARAMS['DB']['TABLE']}', {$row['EDIT_ID']}, this)">
