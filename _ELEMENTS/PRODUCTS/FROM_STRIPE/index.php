					...
					// Append each value wrapped in <td> tags to the current row
						...
						switch ($col) {
							...
							*/
								...
								$htmlOutput .= <<<NAME
														<td class="   ">
															...
															<div class="d-flex justify-content-between  gap-3">
																...
																<div id="" class="w-50" style="">   
																	<table class="text-sm text-secondary-light">
																		<tbody>
																			<tr>
																				<td>Producto: </td>
																				<td class="ps-8 fw-semibold">{$row['name']}</td>
																			</tr>

																			<tr>
																				<td class="">Descripción</td>
																				<td class="ps-8 d-flex flex-wrap">{$row['description']}</td>
																			</tr>
																	...
																	<table class="text-sm text-secondary-light">
																		<tbody>
																			...
																			<tr>
																				...
																				<td class="ps-8">{$row['default_price']}</td>
																			</tr>
							...
							case "default_price":
								if ($row[$col] != ''){
									$price_data = getPriceById($row[$col]);
									$price_data = json_decode($price_data,true);
									$price_data = $price_data['data'];
									
									//$price_data = $price_data['data'];
									//print_r( $price_data);die;
									$price_money = NUMBER_2_MONEY($price_data['unit_amount_decimal']);
									$htmlOutput .= '<td>' . '<span class="sin-wrap">' .  $price_money . '</span>' . '</td>' . "\n";
								}else{
								...
								$htmlOutput .= <<<H
												<td class="text-center ">
													<div class="d-flex align-items-center gap-10 justify-content-center">
														...
														<button type="button" class="bg-danger-focus text-danger-600 bg-hover-danger-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle remove-item-btn" onclick="DELETE_STRIPE('{$T_PARAMS['DB']['TABLE']}', '{$row['id']}', this)">
